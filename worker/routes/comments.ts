...
import { extractMentions, excerpt, uuid } from '../lib/db';
...
commentsRoutes.post('/:id/comments', async (c) => {
  ...
  if (body.parent_id) {
    const parent = await c.env.DB
      .prepare('SELECT question_id, author_email FROM comments WHERE id = ?')
      .bind(body.parent_id)
  ...
  await c.env.DB
    ...
    .bind(commentId, questionId, body.parent_id || null, email, contentStr, now, now)
    .run();
...
commentsRoutes.patch('/:qid/comments/:cid', async (c) => {
  ...
  const existing = await c.env.DB
    .prepare('SELECT author_email FROM comments WHERE id = ? AND deleted_at IS NULL')
    .bind(cid)
    .first<{ author_email: string }>();

  if (!existing) return c.json({ error: 'not found' }, 404);
  if (existing.author_email !== email) return c.json({ error: 'forbidden' }, 403);

  await c.env.DB
    .prepare(
      'UPDATE comments SET content_json = ?, updated_at = ? WHERE id = ?'
    )
    .bind(JSON.stringify(body.content_json), Date.now(), cid)
    .run();
