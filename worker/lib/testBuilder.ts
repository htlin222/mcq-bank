...
export const ALL_STATUS: TestStatus[] = [
	"unseen",
	"wrong",
	"correct",
	"bookmarked",
...
export type RawTestFilters = Partial<{
	...
	tags: string[];
	/**
	 * 明確的題號清單。搜尋結果「把這些出成一份測驗」用的就是它 —— 那一頁的條件是
	 * 全文檢索,沒有辦法用 status/year/group/tag 表達出來,只能把結果本身送過來。
	 *
	 * **它跟其他條件是 AND**,不是取代:於是「從這批搜尋結果裡,只出我答錯的那些」
	 * 這種組合是免費得到的。
	 */
	ids: string[];
	count: number;
...
/** 一次最多接受幾個明確題號。搜尋一次最多回 100 筆,留一點餘裕就夠。 */
...
export function normalizeFilters(raw: RawTestFilters | undefined): TestFilters {
	...
	const rawCount =
		typeof r.count === "number" && Number.isFinite(r.count)
			...
			: DEFAULT_COUNT;
	return {
		status: strList(r.status).filter((s): s is TestStatus =>
			(ALL_STATUS as string[]).includes(s),
		),
		years: intList(r.years),
		groups: strList(r.groups),
		tags: strList(r.tags),
		ids: strList(r.ids).slice(0, MAX_IDS),
...
export function buildTestFilter(
	...
	if (f.ids.length > 0) {
		clauses.push(`q.id IN (${f.ids.map(() => "?").join(",")})`);
		params.push(...f.ids);
	}
	if (f.tags.length > 0) {
		clauses.push(
