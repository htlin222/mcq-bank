...
test("狀態複選是 OR 語意,包在同一組括號裡", () => {
	const f = buildTestFilter(
		normalizeFilters({ status: ["unseen", "bookmarked"] }),
		"a@b.c",
	);
...
test("normalizeFilters:題數夾在 1..100、未知狀態剔除、空陣列視為不限", () => {
	...
	assert.deepEqual(
		normalizeFilters({ status: ["unseen", "bogus"] as never }).status,
		["unseen"],
	);
...
test("normalizeFilters:去重、剔除非法型別、年份轉整數", () => {
	assert.deepEqual(normalizeFilters({ status: ["wrong", "wrong"] }).status, [
		"wrong",
	]);
	assert.deepEqual(
		normalizeFilters({ years: [114, 114, "113" as never] }).years,
		[114, 113],
	);
	assert.deepEqual(
		normalizeFilters({ groups: ["內科", "", 3 as never] }).groups,
		["內科"],
	);
	assert.deepEqual(normalizeFilters({ tags: ["  AML  ", "AML"] }).tags, [
		"AML",
	]);
...
// 明確題號清單 —— 搜尋結果「把這些出成一份測驗」用的。那一頁的條件是全文檢索,
// 沒辦法用 status/year/group/tag 表達,只能把結果本身送過來。
test("ids 產生 IN 片段,而且與其他條件是 AND 不是取代", () => {
	const f = normalizeFilters({
		ids: ["113-001", "113-002"],
		status: ["wrong"],
	});
	const { whereSql, params } = buildTestFilter(f, "me@example.com");
	assert.match(whereSql, /q\.id IN \(\?,\?\)/);
	// AND:於是「從這批搜尋結果裡,只出我答錯的那些」是免費得到的。
	assert.match(whereSql, / AND /);
	assert.deepEqual(params.slice(-2), ["113-001", "113-002"]);
});

test("ids 去重、剔除非法型別,並夾在 MAX_IDS 以內", () => {
	const f = normalizeFilters({
		ids: ["113-001", "113-001", 42 as unknown as string, "  113-002  "],
	});
	assert.deepEqual(f.ids, ["113-001", "113-002"]);
	const many = normalizeFilters({
		ids: Array.from({ length: MAX_IDS + 50 }, (_, i) => `q-${i}`),
	});
	assert.equal(many.ids.length, MAX_IDS);
});

test("ids 為空時不產生任何片段 —— 空陣列一律代表「不限」", () => {
	const { whereSql } = buildTestFilter(
		normalizeFilters({ ids: [] }),
		"me@example.com",
	);
	assert.ok(!/q\.id IN/.test(whereSql), whereSql);
});
