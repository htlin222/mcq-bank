...
export function ExamResult() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
      ...
      {/* Score banner */}
      <div className="bg-white border border-ink-200 rounded-lg p-6 sm:p-8 shadow-paper mb-8 text-center">
        <div className="text-sm text-ink-500 mb-2">
          {data.session.year} 年度模擬考
        </div>
        <div className="font-serif text-6xl text-ink-900 mb-3">
          {correct}<span className="text-ink-300 text-3xl">/{total}</span>
        </div>
        <div className={`text-lg font-medium ${pct >= 60 ? 'text-emerald-700' : 'text-rose-700'}`}>
          {pct}%
        </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-ink-500 mt-1">
                    {unanswered ? (
                      ...
                      <span className="text-emerald-700">✓ {a.chosen}</span>
                    ) : (
