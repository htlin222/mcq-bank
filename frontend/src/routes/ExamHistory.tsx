        ...
        <ul className="space-y-2">
          {rows.map((s) => {
            ...
            return (
              <li
                key={s.id}
                className="flex items-stretch gap-2 bg-white border border-ink-200 rounded hover:border-accent hover:shadow-paper transition"
              >
                <Link
                ...
                <button
                  ...
                  disabled={busy === s.id}
                  aria-label="刪除這筆紀錄"
                  className="px-3 text-ink-400 hover:text-rose-600 disabled:opacity-40"
                >
                  <Trash2 size={16} />
                </button>
              </li>
