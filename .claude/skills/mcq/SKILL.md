...
name: mcq
version: 0.6.0
description: 血液腫瘤考古題單題(題幹/選項/答案/共筆詳解/個人筆記,筆記可讀可寫,可從 HTML 或 OpenEvidence 公開對話匯入圖文筆記);也能用關鍵字搜尋題目找出題號。當使用者輸入 /mcq 年-題號(例如 /mcq 114-001、也接受 114-1),或用 /mcq 關鍵字(例如 /mcq CML)搜尋時使用。
---
...
使用者給的不是題號、而是主題關鍵字(例:`/mcq CML`、「幫我找 CMV 的題」),就用搜尋找出題號:

    python3 "$SKILL/scripts/get_mcq.py" --search CML          # 也可 --year 113 / --limit 30

`get_mcq.py CML`(直接把關鍵字當 positional)也會自動辨識成搜尋。輸出是一串 `年-題號 [group] snippet`(命中的關鍵字用【】標出),**不含答案**。把清單呈現給使用者,請他挑一個題號,再照上面的作答/揭曉流程走。

**關鍵字要用縮寫,不要打全名。** 搜尋是 FTS 前綴比對再把每個 token 用 AND 串起來:

- ✅ 好:`CML`、`CMV`、`AML`、`DIC`、`ITP`、`Factor VIII` — 短、命中率高
- ❌ 不好:`chronic myeloid leukemia` — 會變成 `chronic* myeloid* leukemia*` 三個詞**全部都要出現**才命中,反而找不到

所以先用最短的縮寫或單一關鍵字搜;沒結果再換別的縮寫,而不是改打全名。中文詞(例:`血栓`)可直接打。
