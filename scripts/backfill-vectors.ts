...
async function main() {
  ...
  console.log(`🧮 Backfill vectors → Vectorize (${mode}${dryRun ? ', DRY-RUN' : ''})`);
  ...
  const rows: QRow[] = JSON.parse(listJson)?.[0]?.results ?? [];
  if (rows.length === 0) {
  ...
  for (let i = 0; i < targets.length; i += EMBED_BATCH) {
    ...
    console.log(`   embedded ${Math.min(i + EMBED_BATCH, targets.length)}/${targets.length}`);
  }

  if (dryRun) {
    console.log('\n🔎 DRY-RUN sample:');
    for (const r of records) {
      console.log(`   ${r.id}  dims=${r.values.length}  meta=${JSON.stringify(r.metadata)}`);
    }
