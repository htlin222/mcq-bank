...
async function main() {
	...
	for (let i = 0; i < qRows.length; i += EMBED_BATCH) {
		...
		console.log(
			`   embedded ${Math.min(i + EMBED_BATCH, qRows.length)}/${qRows.length}`,
		);
	}
	...
	const allTags = [...tagToQuestions.keys()].sort(
		(a, b) => b.length - a.length,
	);
	console.log(
		`   ${allTags.length} distinct tag(s) across ${tagRows.length} row(s).`,
	);
	...
	if (slugRows.length === 0) {
		console.error(
			slugFilter
				? `❌ No kind='lecture' doc with slug "${slugFilter}".`
				: "❌ No kind='lecture' docs found.",
		);
		process.exit(2);
	}
	...
	for (const { slug } of slugRows) {
		...
		if (dryRun) {
			console.log(
				`   DRY-RUN: would write ${rows.length} link(s) for ${slug} (no D1 write).`,
			);
			continue;
		}
		...
		const now = Date.now();
		const statements = [
			`DELETE FROM lecture_page_questions WHERE slug = '${escape(slug)}';`,
		];
		for (const r of rows) {
			statements.push(
...
async function recallCandidates(
	...
	if (keywords.length > 0) {
		const ftsExpr = keywords
			.map((t) => (/^[A-Za-z0-9]+$/.test(t) ? `${t}*` : `"${t}"`))
			.join(" OR ");
	...
	for (const t of hitTags) {
		for (const id of (tagToQuestions.get(t) ?? []).slice(0, RECALL_K))
			push(id, "tag");
	}
...
async function judge(
	...
	for (let attempt = 0; attempt < 2; attempt++) {
		try {
			...
			if (!json.success)
				throw new Error(`judge error: ${JSON.stringify(json.errors ?? json)}`);
			...
			if (attempt === 1) {
				console.warn(
					`   ⚠ judge failed for ${q.id}, treating as not relevant: ${String(e)}`,
				);
				return null;
			}
		}
...
async function d1Json<T>(command: string, mode: string): Promise<T[]> {
	const out = await shCapture("wrangler", [
		"d1",
		"execute",
		D1_DB,
		mode,
