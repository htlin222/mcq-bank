...
async function main() {
	...
	if (broken.length > 0) {
		...
		for (const b of broken) {
			console.error(
				`  #${b.number}: ans="${b.answer}", opts=${Object.keys(b.options).length}, stem_start="${b.stem.slice(0, 60)}…"`,
			);
		}
		process.exit(2);
	}
	...
	for (const r of all) {
		...
		if (r.group !== expectedGroup) {
			throw new Error(
				`#${r.number} group mismatch: got ${r.group}, want ${expectedGroup}`,
			);
		}
		const opt = r.options;
	...
	// -------- Explanations SQL --------
	// Each explanation_md → one or more TipTap paragraphs. We split on blank
	// lines first, then on Chinese full-stops 。 (keeping the punctuation),
	// and parse **bold** markdown into bold marks. The UPDATE is guarded so
	// any human edit (version >= 2 or non-system updated_by) is preserved.
	const now = Date.now();
	const stmts: string[] = [
...
function mdToTiptapDoc(md: string) {
	// Step 1: split on blank lines for explicit paragraph breaks the writer
	// intended. Step 2: within each block, split on Chinese full-stop 。 so a
	// single dense sentence-string still renders with paragraph breaks. The
	// lookbehind/lookahead keeps 。 attached to the preceding sentence.
	const blocks = md
		.split(/\n\s*\n/)
		.map((p) => p.trim())
		.filter(Boolean);
...
function parseInlineMarks(text: string) {
	// Only handles **bold** for now — the seed corpus uses no other inline mark.
	const out: any[] = [];
	const re = /\*\*([^*\n]+)\*\*/g;
	let lastEnd = 0;
	let m: RegExpExecArray | null;
	while ((m = re.exec(text)) !== null) {
		if (m.index > lastEnd)
			out.push({ type: "text", text: text.slice(lastEnd, m.index) });
