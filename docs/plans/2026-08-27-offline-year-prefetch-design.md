...
## 二期:圖片(已完成,2026-08-27)

年份頁一顆「連圖片一起離線備用(約 148 張 · 約 9.3 MB)」。**不自動拓** —— 一年
8–17 MB 在行動網路上是真的錢,跟文字那一期差 40 倍,該分開處理。

- **圖片清單不必再打一輪 API。** 文字那一期已經把整年的 payload 放進
  `api-json-v1`,共筆詳解就在裡面,直接讀快取。
- **只看共筆詳解,不看個人筆記。** 後者是私人截圖,量沒有邊界;詳解才是「這一年
  的教材」,也是離線讀的時候真正會看的東西。
- 平均 66 KB/張是實測(2026-08-27 抽 10 張,18.7–147 KB),有一條測試在有人順手
  調整那個常數時會紅 —— 按鈕上的數字是使用者決定要不要按的依據。

### ⚠️ 做這一期才查出來:`img-v1` 從 PWA 上線起就是空轉的

`/img/*` 那條 CacheFirst 路由跟 API **共用同一個 `authGuard`**,而
`isCacheableApiResponse` 要求 `content-type` 含 `application/json` —— 圖片是
`image/webp`,於是 `cacheWillUpdate` 每次都回 null。實測(修正前):頁面上有兩張
`/img/` 圖、直接 fetch 回 200 `image/png`,而 `caches.keys()` 裡**連 `img-v1` 都
不存在**。

**它能活這麼久,是因為症狀跟「圖片本來就沒預載」長得一模一樣**(離線看詳解沒有
圖)。在做二期之前,沒有人有理由期待它有圖。

修法是 `isCacheableImageResponse`,防 Access 302 的那一層完全共用 —— CacheFirst
存下一頁 `text/html` 的登入頁,那張圖的位置會永遠是那頁 HTML,而且它不會再去問
網路。`maxEntries` 300 → 2000(一年就 125–259 張,300 連一年都不夠)、
`maxAgeSeconds` 30 天 → 365 天(key 是 UUID,內容永遠不變)。

**發現之後順手掃過同一類「設定了但永遠不會生效」的東西**:把前端出現過的 146 種
`/api/` 路徑逐一對 `CACHEABLE_API` 的 6 條規則測過,全部都真的會命中 ——
`img-v1` 那個不是更大模式的一部分。(掃描器對 `` `/api/lectures${q}` `` 這種
query 後綴的樣板會誤報,所以那次結果要人看過才算數,沒有把它做成常態測試。)
