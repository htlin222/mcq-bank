...
**第一版**:考古題 203 題 + ASH 補充(~300)+ 判定 + 兩種模式 + 成績/錯題本 +
詳解骨架(~100 份)+ 「這個寫法也該算對」提報投票。

**延後**:PathologyOutlines 補充圖。它需要先解出上百個 topic slug、再用
gentle scraper 慢慢抓(每個 4–8 秒),而且對純 RBC 形態那一類幾乎沒有覆蓋 ——
第一版沒有它照樣完整。資料模型已經留好位置(`smear_questions.source='po'`),
補進來不需要改 schema。
