...
### 模擬考與複習進度: 兩個表各說各話,而使用者只看得到不一致

回報的原話是「我在考試當下明明就是選對的,他對答案,卻用的是我在複習 mode 時的
答案」。查下去不是資料被污染 —— `exam_answers` 與該場 `attempts` 100 題全部一致。
是兩個表回答不同的問題,而 UI 只讀其中一個:

| | |
|---|---|
| `/q/:id` 的「我的作答」 | 讀 `review_progress`(`worker/routes/questions.ts` 的 `my_progress`) |
| 模擬考寫哪裡 | `exam_answers` + `attempts` —— **`review_progress` 在 `worker/routes/exam.ts` 出現 0 次** |

所以考完之後打開任何一題,看到的是**上一次在複習模式**答的那個,連 ✓/✗ 都是舊的。
正式機那場實測有 12 題是「考試答對、題目頁顯示複習時答錯的那個」,另有反方向的
(考錯,題目頁顯示一個月前複習時答對的 A)。

**修法是成績頁一顆明確的「登記進複習進度」,不是讓 `/q/:id` 自動讀最新一筆
attempt。** 後者看起來更聰明,但會**靜靜弄壞「清除本題作答紀錄」** ——
`reviewRoutes` 的 `DELETE /answer/:id` 只刪 `review_progress`,`attempts` 原封不動,
於是清完重整,答案又回來了。保住它就得再加墓碑欄位,而 `attempts` 是全站作答真相、
不能刪。明確按鈕讓複習 UI 繼續只讀 `review_progress` 一個地方。

- **只登記考對的**(`worker/lib/apply-exam-to-review.ts`)。複習紀錄因此維持
  「目前最好的狀態」,不會因為一次考差把以前答對的拉下來。
- **只動 `last_chosen` / `last_correct` / `last_seen_at`,不碰 `times_seen` /
  `times_correct`。** 那兩個餵的是到期佇列與熟練度統計,一次批次登記把它們整批
  加一,等於用一個看起來只是「同步顯示」的按鈕去重排整個複習排程。但
  `last_correct` 本身就會讓題目離開 `/weakness-map` 的錯題池
  (`last_correct = 0 OR times_correct * 2 < times_seen`)—— 那是這個功能要的。
- **新列的 `times_seen` 是 0 而不是 1。** 那個欄位是「複習模式看過幾次」,而這題
  確實一次都沒有(它是在模擬考裡答的)。欄位各自說各自的話,不互相冒充。
- **逐題那顆只在「考對且複習紀錄不一致」時出現** —— 一顆按了不會有任何變化的按鈕
  比沒有更糟。已登記的留一行灰字,不留的話按完只是按鈕消失,看起來像壞掉。批次
  按鈕上的數字是「按下去會改變幾題」,**與伺服器同一套判準**(前端自己算一套的話,
  按完會發現數字沒動)。

⚠️ **查這類「哪個表說謊」的問題時,空結果先驗儀器。** 這次有一步用錯 email 去
join,拿到 0 筆,差一點就得出「沒有這回事」的結論 —— 真正的擁有者是
`hsieh.ting.lin@gmail.com` 而不是登入用的那個。同 `users_online.json` 空 fixture
那條:空掃永遠是綠的。

⚠️ **e2e 在這一頁有兩個會誤導的坑。** 成績頁預設篩選是「答錯/未答」,所以考對的
題目根本不在畫面上 —— 量到 0 顆按鈕會被誤讀成「功能沒做出來」,要先切到「全部」。
以及 **Playwright 後註冊的 route 先比對**:擋外連的 catch-all 要**先**掛,專用那條
後掛,反過來的話專用那條收不到請求,而症狀只是「送出的題號不對」。

### 交卷: 一趟請求,不是一題一趟

`submit()` 曾經逐題重送答案(`for (...) await api.post('/answer')`),100 題就是
100 趟循序往返,而 `/answer` 內部還有兩趟 D1。正式機實測**每趟 1.30 秒** ——
交卷要等兩分鐘以上,回報寫的是「卡在交卷中送不出去」。量到的證據:交卷那一波
146 筆 attempts(其中 **144 筆 `elapsed_ms IS NULL`** —— 只有重送迴圈會這樣寫)
涵蓋 100 題、跨 187.7 秒,144 ÷ 100 表示使用者中途重來過一次。

現在走 `PUT /api/exam/:sid/answers`:一次 ownership 檢查 + 一個 `DB.batch()`。
`/start` 本來就在用 101 條 statement 的 batch,規模是驗證過的。

- **仍然全量送,不是只送「debounce 還沒觸發」的那幾題。** 作答時那支 POST 沒有
  重試也沒有 `catch`,失敗是無聲的,安全網要蓋得住;改成一趟之後,蓋滿的成本是零。
- **只有真的改變的答案才 append `attempts`**(`worker/lib/exam-answers.ts`)。舊路徑
  對 100 題全部 append,於是每場考試在 `attempts` 裡多出 100 筆 `elapsed_ms` 為
  NULL 的重複列 —— 舊程式自己的註解就寫著「這不是一次新的作答事件」,卻仍把它記成
  事件。
- **`/state` 對已交卷的 session 回 410,前端要處理。** 舊版只在「沒有快取」時才處理,
  而 sessionStorage 快取只有交卷成功才清 —— 於是交卷後重整,畫面用舊快取渲染成
  「還在考試」,按幾次交卷都只會被 400。那也是「卡在交卷」的一種。
- **`submit()` 要有 `catch`。** 舊版 `try/finally` 讓任何失敗都只是把按鈕默默彈回
  「交卷」,rejection 連 console 都只留一行 unhandled。
- 守門的 e2e 數的是**請求數**不是時間:時間在測試環境量不準(fixture 伺服器沒有 D1
  的往返成本),而請求數是那個成本的來源,且是確定性的。
